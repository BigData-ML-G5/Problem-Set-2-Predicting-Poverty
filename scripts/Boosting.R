...
p_load(
  ...
  xgboost    # XGBoost (caret method = "xgbTree")
...
# -------------------------------
# 9) AdaBoost (adabag::AdaBoost.M1 via caret)
# -------------------------------
adagrid <- expand.grid(
  mfinal    = c(50, 100, 200),
  maxdepth  = c(1, 2, 5),
...
ada_thr <- find_best_threshold(Ada_model)
cat(sprintf("[AdaBoost] Best CV-F1 = %.4f at threshold = %.2f\n", ada_thr$F1, ada_thr$thresh))

# -------------------------------
# 10) GBM (gbm) via caret
# -------------------------------
...
# -------------------------------
# 12) Keep ONLY the best model and write a single Kaggle CSV
# -------------------------------
# (Optional) Remove old CSVs from previous runs (to avoid confusion)
cleanup_previous <- TRUE
if (cleanup_previous) {
  old <- list.files(pattern = "^(ADA|GBM|XGB)_.*\\.csv$")
  if (length(old)) file.remove(old)
}

cv_scores <- tibble(
  alg = c("ADA","GBM","XGB"),
  F1  = c(ada_thr$F1, gbm_thr$F1, xgb_thr$F1)
) |>
  arrange(desc(F1))
...
if (best_alg == "ADA") {
  bt <- Ada_model$bestTune
  prob <- predict(Ada_model, newdata = test, type = "prob")[, "Yes"]
  thr  <- ada_thr$thresh
  fname <- mkfile("ADA", c(
    paste0("mfinal_",   bt$mfinal),
    paste0("maxdepth_", bt$maxdepth),
    paste0("coef_",     bt$coeflearn),
    paste0("th_",       fmt_dec(thr))
  ))
} else if (best_alg == "GBM") {
  bt <- GBM_model$bestTune
  prob <- predict(GBM_model, newdata = test, type = "prob")[, "Yes"]
  thr  <- gbm_thr$thresh
  fname <- mkfile("GBM", c(
  ...
  prob <- predict(XGB_model, newdata = test, type = "prob")[, "Yes"]
  thr  <- xgb_thr$thresh
  fname <- mkfile("XGB", c(
...
cat("Submission CSV created. Upload this to Kaggle -> ", fname, "\n", sep = "")
